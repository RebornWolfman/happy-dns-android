...
public final class DnsManager {
    ...
    public volatile NetworkInfo info = null;
     ...
     */
    public String[] query(String domain, android.net.Network net) throws IOException {
        return query(new Domain(domain), net);
    }

    public String[] query(Domain domain, android.net.Network net) throws IOException {
        if (domain == null) {
        ...
        String[] r = queryInternal(domain, net);
    ...
    private String[] queryInternal(Domain domain, android.net.Network net) throws IOException {
//        有些手机网络状态可能不对
...
//        }
        ...
        synchronized (cache) {
            if (info.equals(NetworkInfo.normal) && Network.isNetworkChanged(net)) {
                cache.clear();
        ...
        for (int i = 0; i < resolvers.length; i++) {
            ...
            String ip = Network.getIp(net);
            try {
                records = resolvers[pos].resolve(domain, info, net);
    ...
    public InetAddress[] queryInetAdress(Domain domain, android.net.Network net) throws IOException {
        String[] ips = query(domain, net);
