...
public class QiniuDns implements IResolver{
    ...
    @Override
    public Record[] resolve(Domain domain, NetworkInfo info, android.net.Network net) throws IOException {
        HttpsURLConnection connection = null;
        if(net == null){
            connection = (HttpsURLConnection) new URL(ENDPOINT + mAccountId
            ...
            URL url = new URL(ENDPOINT + mAccountId
                    + "/d?dn=" + (mEncryptKey == null ? domain.domain
                    : DES.encrypt( domain.domain
                    + "?e=" + Long.toString((System.currentTimeMillis()
                    + mExpireTimeMs) / 1000), mEncryptKey))
                    + "&ttl=1" + "&echo=1");
            connection = (HttpsURLConnection) net.openConnection(url);
        }
