...
//        the system dns ip would change after network changed.
        return new IResolver() {
            @Override
            public Record[] resolve(Domain domain, NetworkInfo info, Network net) throws IOException {
                InetAddress[] addresses = null;
                if (info.dnsServers != null && !info.dnsServers.isEmpty()){
                    addresses = info.dnsServers.toArray(new InetAddress[]{});
                }
                if (addresses == null){
                    addresses = getByReflection();
                }
                if (addresses == null) {
                ...
                Record[] records = resolver.resolve(domain, info, net);
                if (domain.hasCname) {
                    boolean cname = false;
